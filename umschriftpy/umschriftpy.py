...
class UmschString(array.array):
    def to_unicode(self, flags=0):
        ...
        if flags & ExportRules.SUPRESS_I_WITH_DIAERESIS:
   
            copy = copy.replace(UmschString('L', [PSEUDO_I_WITH_DIAERESIS]), UmschString('L', [PSEUDO_Y])).replace(UmschString('L', [PSEUDO_i_WITH_DIAERESIS]), UmschString('L', [PSEUDO_y]))      
  
        if flags & ExportRules.JJ_FOR_DOUBLE_YOD:
            copy = copy.replace(UmschString('L', [PSEUDO_y]), UmschString('L', [ASC_j, ASC_j])).replace(UmschString('L', [PSEUDO_Y]), UmschString('L', [ASC_J, ASC_j])).replace(UmschString('L', [PSEUDO_I_WITH_DIAERESIS]), UmschString('L', [ASC_J, ASC_j])).replace(UmschString('L', [PSEUDO_i_WITH_DIAERESIS]), UmschString('L', [ASC_j, ASC_j]))      
...
def export_string(input, output_format, flags = 0):
    ...
    if flags & ExportRules.K_WITH_DOT:
        export_dict.update(EXPORT_DICT_K_WITH_DOT[output_format])
    if flags & ExportRules.J_FOR_YOD:
